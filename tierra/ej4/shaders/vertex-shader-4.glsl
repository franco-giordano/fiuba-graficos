        ...
        void main(void) {
           ...
           vec4 textureColor = texture2D(uSampler, vec2(uv.s, uv.t));         
            ...
            if (textureColor.b > 1.3*textureColor.r && textureColor.b > 1.3*textureColor.g) {
                // agua
                position += sin(25.*position.x + time*30.)*.008 + cos(25.*position.y + time*30.)*.008;
            }
            else {
                // continente
                position*=1.05;
            ...
            // position.x = .5*cos(2.*PI*position.x);
            // position.z = .5*sin(2.*PI*temp);
            // position.y *= 2.;

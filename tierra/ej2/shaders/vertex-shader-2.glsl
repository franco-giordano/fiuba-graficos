        ...
        void main(void) {
           ...
           vec4 textureColor = texture2D(uSampler, vec2(uv.s, uv.t));         
            ...
            position = vec3(uv.xy, 1.);
            position -= vec3(.5);
            
            position.z += sin(position.z + time*20. - 140.*length(position))*10.*(exp(-25.*(length(position)-0.3)));

            float temp = position.y;
            position.y = cos(PI/2.0)*position.y - sin(PI/2.0)*position.z;
            position.z = sin(PI/2.0)*temp + cos(PI/2.0)*position.z;
